...
def compute_sequence_level_advantage(
    token_level_rewards: torch.Tensor,
    values: torch.Tensor, # V(s_prompt) logits, shape (bs,)
    response_mask: torch.Tensor,
    config: Optional[AlgoConfig] = None,
    epsilon: float = 1e-8, # Epsilon for normalization stability
    **kwargs,
) -> Tuple[torch.Tensor, torch.Tensor]:
    with torch.no_grad():
        R = token_level_rewards.sum(dim=-1) # (bs,)
        V_s_prompt_logit = values # (bs,)
        V_s_prompt_prob = torch.sigmoid(V_s_prompt_logit)
        adv = R - V_s_prompt_prob # (bs,)
        current_batch_size = adv.shape[0]
        adv_mean_before = torch.mean(adv)
        adv_std_before = torch.std(adv, unbiased=False)
        advantages = adv.unsqueeze(-1) * response_mask
        returns_target_R = R # (bs,)
